...
class DeepFeatureNet(object):
    ...
    def _build_placeholder(self):
        ...
        self.input_var = tf.placeholder(
            ...
            name=name + "_inputs"
        ) # use tf.nn.conv_2D so input size = [batch, in_height, in_width, in_channels]
        # Target
...
#       Ben: changed to the line below due to 1.6 changed concat order
...
class DeepSleepNet(DeepFeatureNet):
    ...
    def build_model(self, input_var):
        ...
        with tf.variable_scope(name) as scope:
            ...
            bw_lstm_cell1 = tf.nn.rnn_cell.LSTMCell(hidden_size, use_peepholes=True, state_is_tuple=True)
            bw_lstm_cell2 = tf.nn.rnn_cell.LSTMCell(hidden_size, use_peepholes=True, state_is_tuple=True)
            if self.use_dropout_sequence:
                keep_prob = 0.5 if self.is_train else 1.0
                fw_lstm_cell1 = tf.nn.rnn_cell.DropoutWrapper(fw_lstm_cell1, output_keep_prob=keep_prob)
                fw_lstm_cell2 = tf.nn.rnn_cell.DropoutWrapper(fw_lstm_cell2, output_keep_prob=keep_prob)
                bw_lstm_cell1 = tf.nn.rnn_cell.DropoutWrapper(bw_lstm_cell1, output_keep_prob=keep_prob)
                bw_lstm_cell2 = tf.nn.rnn_cell.DropoutWrapper(bw_lstm_cell2,output_keep_prob=keep_prob)

            fw_cell = tf.nn.rnn_cell.MultiRNNCell([fw_lstm_cell1, fw_lstm_cell2], state_is_tuple=True)
            bw_cell = tf.nn.rnn_cell.MultiRNNCell([bw_lstm_cell1,bw_lstm_cell2], state_is_tuple=True)
            # Initial state of RNN
            self.fw_initial_state = fw_cell.zero_state(self.batch_size, tf.float32)
            self.bw_initial_state = bw_cell.zero_state(self.batch_size, tf.float32)
            list_rnn_inputs = tf.unstack(seq_input, axis=1)
            outputs, fw_state, bw_state = tf.nn.static_bidirectional_rnn(cell_fw=fw_cell, cell_bw=bw_cell,
                                                                         inputs=list_rnn_inputs,
                                                                         initial_state_fw=self.fw_initial_state,
                                                                         initial_state_bw=self.bw_initial_state)
            ...
            if self.return_last:
                ...
                network = tf.reshape(tf.concat(values=outputs, axis=1), [-1, hidden_size*2],
                                     name=name)
            self.activations.append((name, network))
            self.layer_idx +=1
